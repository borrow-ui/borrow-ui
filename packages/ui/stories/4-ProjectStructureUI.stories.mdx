...
<Meta title="Project Structure|ui package" />
...
-   storybook for documentation and, in combination with the
    [storyshot addon](https://github.com/storybookjs/storybook/tree/master/addons/storyshots), for snapshots tests;
...
-   `.storybook` and `stories` folder are Storybook related configuration and main stories.
...
While `.storybook` and `stories` folder are pretty straightforward to understand, the `src` folder deserves documentation.
