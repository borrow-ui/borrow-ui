...
export function NavbarControls({ toggleBodyOpen, floating = false }) {
    const controlsClassName = `${NAVBAR_CONTROLS_CLASS} ${floating ? NAVBAR_CONTROLS_FLOATING_CLASS : ''}`;

    return (
        <div className={controlsClassName}>
            <span onClick={() => toggleBodyOpen(false)} style={{ cursor: 'pointer', fontSize: '24px' }}>
                &times;
            </span>
        </div>
    );
