...
describe('Modal', () => {
    test('renders trigger and content', async () => {
        render(
            <Modal
                Trigger={({ setVisible }) => (
                    <span onClick={() => setVisible((v: boolean) => !v)} data-testid="trigger">
                        Click to open
                    </span>
                )}
                getModalWindowProps={() => ({
    ...
    test('calls onWrapperClick', async () => {
        ...
        const wrapperElement: HTMLElement | null = screen.queryByTestId('wrapper');
        if (wrapperElement !== null) await userEvent.click(wrapperElement);
