...
describe('ReactSelect', () => {
    test('renders and can be changed', async () => {
        const Form = () => {
            ...
            return (
                <>
                    <ReactSelect onChange={(v) => setValue(v.value)} options={options} />
        ...
        await userEvent.type(screen.getByRole('textbox'), 'Th');
    ...
    test('renders with an initial values in multi select and can be changed', async () => {
        const Form = () => {
            ...
            return (
                <>
                    <ReactSelect
                        isMulti={true}
                        onChange={(vs) => setValues(vs.map((v) => v.value))}
                        options={options}
                        value={values}
                    />
        ...
        await userEvent.type(screen.getByRole('textbox'), 'Fou');
    ...
    test('uses CreatableSelect', async () => {
        ...
        await userEvent.type(screen.getByRole('textbox'), 'New One');
